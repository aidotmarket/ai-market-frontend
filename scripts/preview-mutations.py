...
mutations = [
    ...
    ('skip-attestation-check',
     ...
     'lib/listing-preview/policy.test.ts', 'refuses a signed but incorrectly bound scan-attestation digest'),
    ('skip-fetched-leaf-digest',
     ...
     'lib/listing-preview/policy.test.ts', 'hides all rows when a validly signed attestation names a different fetched leaf set'),
    ('accept-unknown-policy-version',
     ...
     "    && true, 'scan_policy_unknown'); // MUTATION: ignore version.",
     'lib/listing-preview/policy.test.ts', 'accepts only the exact known policy/version pair'),
]
results = []

...
export default function ListingPreview({ draft, saved = false }: { draft: PublicFields; saved?: boolean }) {
  ...
  return <section aria-label="Listing preview" className="space-y-5 rounded-xl border border-indigo-200 bg-indigo-50/40 p-5 sm:p-6">
    <div><p className="text-xs font-semibold uppercase tracking-wide text-indigo-700">Private preview</p><h3 className="mt-2 text-lg font-semibold text-gray-900">Review your listing</h3><p className="mt-2 text-sm leading-6 text-gray-600">Check the wording buyers would see. {saved ? 'This review uses your saved listing fields.' : 'This preview uses your current fields, including any unsaved edits.'}</p></div>
    ...
    <p className="text-sm leading-6 text-gray-600">Your brief, conversation and unaccepted suggestions are private and do not appear here. This preview does not publish your listing.</p>
  </section>;
}

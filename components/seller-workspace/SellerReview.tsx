...
export default function SellerReview({active, enabled}: {active: boolean; enabled: boolean}) {
  ...
  return <section aria-label="Review saved listing" className="space-y-5">
    ...
    {review && <><ListingPreview draft={review.fields} saved />{review.missing_fields.length > 0 && <p role="status" className="rounded-lg bg-amber-50 p-4 text-sm text-amber-900">Complete and save these listing fields: {review.missing_fields.join(', ')}.</p>}<p className="text-sm text-gray-600">Public sample selection and final approval are being connected. This review does not approve or publish your listing.</p></>}
  </section>;
}

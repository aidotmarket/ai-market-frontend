...
export default function SellerReview({active, enabled}: {active: boolean; enabled: boolean}) {
  ...
  return <section aria-label="Review saved listing" className="space-y-5">
    ...
    {review && <>{review.source_files && review.source_files.length > 0 && <ReviewFiles key={review.review_hash} files={review.source_files} />}<section aria-label="Saved listing preview" className="space-y-4 rounded-xl border border-indigo-200 bg-indigo-50/40 p-4 sm:p-6"><p className="text-sm text-gray-600">This review uses your saved listing fields. Your brief, conversation and unaccepted suggestions are private.</p><iframe key={review.render_hash} onLoad={() => setLoadedRender(review.render_hash)} title="Saved listing buyers would see" sandbox="" referrerPolicy="no-referrer" srcDoc={review.rendered_html} className="h-[min(640px,75vh)] min-h-80 w-full rounded-xl border border-gray-200 bg-white" /></section>{review.missing_fields.length > 0 && <p role="status" className="rounded-lg bg-amber-50 p-4 text-sm text-amber-900">Complete and save these listing fields: {review.missing_fields.join(', ')}.{review.missing_fields.includes('price') && ' Enter a price from $0 to $999,999.99, with no more than two decimal places.'}</p>}<p className="text-sm text-gray-600">Publishing uses the exact saved review shown above.</p>{review.approval_available && review.missing_fields.length === 0 && <SellerApproval key={review.review_hash} review={review} active={active} rendered={loadedRender === review.render_hash} />}</>}
  </section>;
}

function ReviewFiles({files}:{files:NonNullable<ListingReview['source_files']>}) {
  ...
  return <section aria-label="Files included in this review" className="rounded-xl border border-gray-200 bg-white p-5">
    ...
    <p className="mt-2 text-sm text-gray-900">{files.length.toLocaleString('en')} files · {files.reduce((sum,file)=>sum+file.size,0).toLocaleString('en')} bytes</p>
    ...
    <ul className="mt-3 divide-y divide-gray-100">{files.slice(page*50,(page+1)*50).map(file=><li key={JSON.stringify([file.key,file.version_id])} className="flex flex-wrap justify-between gap-2 py-2 text-sm"><span className="min-w-0 break-all text-gray-900">{file.key}</span><span className="text-gray-500">{file.size.toLocaleString('en')} bytes</span></li>)}</ul>
    {files.length>50 && <div className="mt-3 flex items-center gap-3 text-sm"><button type="button" disabled={page===0} onClick={()=>setPage(value=>value-1)}>Previous review files</button><span>Showing {page*50+1}–{Math.min((page+1)*50,files.length)} of {files.length.toLocaleString('en')}</span><button type="button" disabled={(page+1)*50>=files.length} onClick={()=>setPage(value=>value+1)}>Next review files</button></div>}
  </section>;
}

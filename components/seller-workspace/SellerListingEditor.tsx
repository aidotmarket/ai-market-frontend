...
export default function SellerListingEditor({ assistant, active = true, initialContent, onSave }: { assistant?: ListingAssistant; active?: boolean; initialContent?: ListingDraftContent; onSave?: (content: ListingDraftContent) => Promise<void> }) {
  ...
  const priceInvalid = price !== '' && (!/^[0-9]{1,12}(?:\.[0-9]{1,2})?$/.test(price) || Number(price) > 999999.99);
  ...
  const save = async () => {
    if (!onSave || savingRef.current || priceInvalid) return;
    ...
    <section className="space-y-5" aria-labelledby="listing-editor-title">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5 rounded-xl border border-gray-200 bg-white p-5 sm:p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-gray-900">Your price (USD)<input type="number" min="0" max="999999.99" step="0.01" aria-invalid={priceInvalid} aria-describedby={priceInvalid ? "seller-price-error" : undefined} value={price} onChange={(event) => setPrice(event.target.value)} className={fieldClass} /></label><label className="text-sm font-semibold text-gray-900">Your license<input value={license} onChange={(event) => setLicense(event.target.value)} maxLength={500} className={fieldClass} /></label></div>
          {priceInvalid && <p id="seller-price-error" role="alert" className="text-sm text-red-800">Enter a price from $0 to $999,999.99, with no more than two decimal places.</p>}
          {onSave ? <div className="space-y-3"><button type="button" disabled={saving || priceInvalid || snapshot === savedSnapshot} onClick={save} className={buttonClass}>{saving ? 'Saving draft…' : 'Save private draft'}</button><p role="status" className="text-sm text-gray-600">{snapshot === savedSnapshot ? 'Draft saved to your account.' : 'You have unsaved changes.'} Saving does not publish your listing.</p>{saveError && <p role="alert" className="text-sm text-red-800">{saveError}</p>}<p className="text-xs text-gray-500">Saves your listing fields, brief, price and license. Chat and unaccepted suggestions stay in this Workspace only. Save your file choices separately in Choose what to sell.</p></div> : <p className="text-xs leading-5 text-gray-500">Your edits stay here when you switch Workspace sections. They are not saved to your account yet and will be lost if you reload or leave the Workspace. Public-sample approval and publishing are not connected yet.</p>}
        </div>

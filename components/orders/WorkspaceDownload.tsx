...
export default function WorkspaceDownload({orderId,requestGrant=requestWorkspaceDownload}: {
  ...
  const download=async () => {
    ...
    if (!picker) return;
    working.current=true;setBusy(true);setError('');setMessage('Choose a folder inside Downloads, or create one in the folder picker.');
  ...
  return <section aria-label="Download purchased files" className="space-y-4 rounded-xl border border-gray-200 bg-white p-6">
    ...
    <div className="rounded-lg border-2 border-amber-500 bg-amber-50 p-4 text-amber-950">
      ...
      <ol className="mt-2 list-decimal space-y-1 pl-5 text-sm">
        ...
        <li>Select that subfolder and allow Chrome to save your files.</li>
      </ol>
      <p className="mt-3 text-sm">If Chrome says “Can’t open this folder”, click <strong>Choose a different folder</strong> and select the subfolder.</p>
    </div>

...
export default function DatasetMembers({ orderId, initialMembers, filesUnavailable = false, unavailableReason = 'files_unavailable', retryable = false, retrying = false, onRetry, accessExpired, ensureTermsAccepted }: {
  ...
  const [loading, setLoading] = useState(!initialMembers && !filesUnavailable);
  ...
  }, [base, initialMembers, filesUnavailable]);
  ...
  async function download(member: DatasetMember) {
    ...
    try {
      ...
      if ('reason' in result) {
        ...
        if (reason === 'member_unavailable') setMembers((rows) => rows.map((row) => row.index === member.index ? { ...row, state: 'unavailable' } : row));
        return;
      }
  ...
  return (
    <section className="rounded-lg border border-gray-200 p-6" aria-label="Files in this dataset">
      ...
      {!filesUnavailable && !loading && members.length === 0 && <p>No files are available yet.</p>}

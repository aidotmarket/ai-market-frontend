...
const STATUS_BADGE: Record<ConversationStatus, { css: string; label: string }> = {
  ...
  seller_replied: { css: 'bg-[#E8EAF6] text-[#303F9F]', label: 'Seller replied' },
            ...
            {inquiries.map((inq) => (
              <tr key={inq.id} className="hover:bg-gray-50">
                <td className="px-4 py-3">
                  <Link href={`/dashboard/inquiries/${inq.id}`} className="text-[#3F51B5] hover:underline font-medium">
                    {inq.listing_title}
                  </Link>
                  {inq.unread_by_buyer > 0 && (
                    <span className="ml-2 inline-flex items-center rounded-full bg-[#3F51B5] px-2 py-0.5 text-xs font-medium text-white">
                      {inq.unread_by_buyer}
                    </span>
                  )}
                </td>
        ...
        {inquiries.map((inq) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              {inq.unread_by_buyer > 0 && (
                <span className="inline-flex items-center rounded-full bg-[#3F51B5] px-2 py-0.5 text-xs font-medium text-white shrink-0">
                  {inq.unread_by_buyer}
                </span>
              )}
            </div>
